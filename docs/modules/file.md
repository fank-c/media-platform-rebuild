...
文件模块（`file-service`）是平台底层多媒体资产管理与对象存储（MinIO）中继协调的核心微服务（运行端口：8040）。负责多模式文件接入（普通表单上传、海量音视频 V2 两阶段暂存直传、内部微服务切片自动托管）、存储物理隔离与原子归档（`staging/` ➔ `permanent/`）、防盗链时效流媒体代理（HMAC 签名与 HTTP 304 协商缓存）以及僵死孤儿文件后台自愈清理。
...
- **大文件 V2 两阶段暂存直传中枢**：
  ...
  - 授权阶段：校验文件 MIME、扩展名与大小，与客户端声明的 SHA-256 校验和绑定，签发仅限直写 MinIO 暂存目录 `staging/` 的短期预签名 PUT 通行证（有效期 5 分钟）；
  - 确认归档阶段：核验客户端上传状态，向 MinIO 发起 HEAD 对象元数据自省，校验 Content-Length 与 ETag/Checksum，校验无误后在 MinIO 内部触发毫秒级 `CopyObject` 移动至正式目录 `permanent/`，并物理删除暂存对象，资产状态置为 `COMPLETED`。
- **存储物理目录严格隔离**：
  - **暂存区（`staging/{authorId}/{date}/{fileId}`）**：任何未经过确认归档的文件物理隔离在此，对业务系统完全不可见；
  - **正式区（`permanent/{authorId}/{date}/{fileId}`）**：只有确认合法的大文件与普通上传文件才允许落入该目录，业务服务仅关联正式区资产。
- **专有内部微服务托管通道**：
...
- **防盗链安全代理与流媒体加速**：
  ...
  - 支持 `ETag` 与 `If-None-Match` 的 HTTP 304 协商缓存，大文件（>10MB）实施熔断保护，杜绝恶意刷流打爆应用服务带宽。
- **存储自愈清理调度**：
  - 通过后台调度器定时扫描超时未确认的直传记录，调用 MinIO 物理删除已上传的废弃分片与垃圾对象，将数据库状态置为 `EXPIRED`。
