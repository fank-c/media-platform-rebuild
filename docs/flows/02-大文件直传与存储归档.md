...
> **历史方案说明：V2 新上传入口已标记过期，不再作为当前接入方案。** 以下为 V2 两阶段直传的设计与存量处理参考，不代表线上已启用或建议新调用方接入。现行阿里云 OSS 适配器不支持 V2 checksum PUT 签名，初始化开关默认关闭；新上传请按 [文件接口契约](../api.md) 走现有 V1 直传或普通上传。存量 V2 记录仍须保留确认、恢复和清理链路；此处不涉及删除接口或更改开关。
...
以下历史 V2 设计采用 **暂存直传架构（Staging-to-Permanent Architecture）**：客户端取得暂存区（`staging/`）的写入权限，服务端完成元数据核验后复制至正式区（`permanent/`）。该设计不再作为当前新上传接入目标。
