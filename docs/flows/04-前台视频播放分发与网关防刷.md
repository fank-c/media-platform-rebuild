...
### 3.4 播放心跳上报与可重复有效播放防重 (`interaction-service`)
前端播放器在播放过程中持续上报心跳 (`POST /api/interactions/videos/{vid}/heartbeat`)：
- **有效播放阈值**：累计有效观看时长达到 5 秒门槛（配置项 `interaction.watch.valid-play-threshold`，默认 5s）；
- **冷却周期与可重复播放**：同一用户针对同一视频按冷却周期（配置项 `interaction.watch.repeat-window`，默认 6h）允许产生可重复的有效播放；
- **数据库行级 CAS 原子抢占**：在满足 5 秒门槛后，通过执行条件更新 `SET last_valid_play_at = now WHERE id = ? AND (last_valid_play_at IS NULL OR last_valid_play_at <= (now - 6h))` 原子抢占播放资格。仅在影响行数为 1 时自增播放量并写入 `PLAY:ACTIVE` Outbox 领域事件，并发请求与冷却期内重复心跳 CAS 返回 0 自动忽略，杜绝虚假刷量与多事件双发。
