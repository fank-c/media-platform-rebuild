...
- **锁与事务分层**：同一 `userId:vid` 的心跳先加 Redisson 分布式锁再开事务，确保提交后才释放锁；等锁超过 3 秒只读降级返回已有进度（本次时长不入账）；
- **完播与计数**：播放头达到 90% 时 CAS 置位 `completed` 并写 `PLAY_COMPLETE` 事件；播放量在事务提交后（`afterCommit`）才写入 Redis 计数。客户端上报的时长与进度目前尚未做可信校验，防刷门禁仍在规划中。
