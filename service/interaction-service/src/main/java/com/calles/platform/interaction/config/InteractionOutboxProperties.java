...
/**
 * 互动微服务事务性发件箱 (Outbox) 运行参数配置类。
 */
@ConfigurationProperties(prefix = "interaction.outbox")
public class InteractionOutboxProperties {

    /** 是否启用发件箱写入总开关；关闭后不持久化 Outbox 记录。 */
    private boolean enabled = true;

    /** 是否启用正式向 RabbitMQ 派发消息的执行总开关；当前推荐端未就绪时默认关闭。 */
    private boolean dispatchEnabled = false;

    /** 是否启用事务提交后的毫秒级快速派发。 */
    private boolean fastDispatchEnabled = false;
